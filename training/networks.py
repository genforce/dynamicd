    ...
    def __init__(self,
        ...
        if in_channels == 0 or architecture == 'skip':
            self.fromrgb = Conv2dLayer(img_channels, tmp_channels, kernel_size=1, activation=activation,
                trainable=next(trainable_iter), conv_clamp=conv_clamp, channels_last=self.channels_last, keepin=True, occupy_flag=not (lidx < keepd), **dynamic_kwargs)
            lidx += 1

        self.conv0 = Conv2dLayer(tmp_channels, tmp_channels, kernel_size=3, activation=activation,
            trainable=next(trainable_iter), conv_clamp=conv_clamp, channels_last=self.channels_last, keepin=keepd==lidx, occupy_flag=not (lidx < keepd), **dynamic_kwargs)
        lidx += 1

        self.conv1 = Conv2dLayer(tmp_channels, out_channels, kernel_size=3, activation=activation, down=2,
            trainable=next(trainable_iter), resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=self.channels_last, keepin=keepd==lidx, occupy_flag=not (lidx < keepd), **dynamic_kwargs)
        lidx += 1

        if architecture == 'resnet':
            self.skip = Conv2dLayer(tmp_channels, out_channels, kernel_size=1, bias=False, down=2,
                trainable=next(trainable_iter), resample_filter=resample_filter, channels_last=self.channels_last, keepin=keepd==lidx, occupy_flag=not (lidx < keepd), **dynamic_kwargs)
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(self,
        ...
        self.conv = Conv2dLayer(in_channels + mbstd_num_channels, in_channels, kernel_size=3, activation=activation, conv_clamp=conv_clamp, occupy_ratio=occupy_ratio, randomly_select=randomly_select, minus_dim=mbstd_num_channels)
        self.fc = FullyConnectedLayer(in_channels * (resolution ** 2), in_channels, activation=activation, occupy_ratio=occupy_ratio, randomly_select=randomly_select, div_dim=(resolution ** 2))
        self.out = FullyConnectedLayer(in_channels, 1 if cmap_dim == 0 else cmap_dim, keepout=True, occupy_ratio=occupy_ratio, randomly_select=randomly_select)
