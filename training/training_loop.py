...
from training.networks import Conv2dLayer, DiscriminatorEpilogue 
...
def training_loop(
    ...
    if dynamic_kwargs is not None:
        current_occupy_ratio = dynamic_kwargs.occupy_start 
    ...
    while True:
        ...
        if (batch_idx % ada_interval == 0) and dynamic_kwargs is not None:
            current_occupy_ratio = ((dynamic_kwargs.occupy_end - dynamic_kwargs.occupy_start) / total_kimg) * (cur_nimg / 1e3) + dynamic_kwargs.occupy_start
            for m in D.modules():
        ...
        if dynamic_kwargs is not None:
            fields += [f"occupy_ratio {training_stats.report0('Progress/occupy_ratio', float(current_occupy_ratio)):.3f}"]
